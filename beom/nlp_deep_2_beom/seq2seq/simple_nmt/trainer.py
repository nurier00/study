...
class MaximumLikelihoodEstimationEngine(Engine):
    ...
    def __init__(self, func, model, crit, optimizer, lr_scheduler, config):
        print('## MaximumLikelihoodEstimationEngine : init')
        self.model = model
        self.crit = crit
    ...
    def train(engine, mini_batch):
        print('## MaximumLikelihoodEstimationEngine : train start ', engine.state.epoch)
        print('## MaximumLikelihoodEstimationEngine : train ( mini_batch :', mini_batch, ')')
        ...
        ## mini_batch.src[0] : vec index
        ## mini_batch.src[1] : sequence length
        ## mini_batch.tgt[0] : vec index
        ## mini_batch.tgt[1] : sequence length
        print('## [MaximumLikelihoodEstimationEngine.train] mini_batch.src :', mini_batch.src)
        print('## [MaximumLikelihoodEstimationEngine.train] mini_batch.tgt :', mini_batch.tgt)
        ...
        print('## MaximumLikelihoodEstimationEngine : train end ', engine.state.epoch)
        return {
            'loss': loss,
            'ppl': ppl,
    ...
    def validate(engine, mini_batch):
        print('## MaximumLikelihoodEstimationEngine : validate')
        engine.model.eval()
    ...
    def attach(
        ...
        print('## MaximumLikelihoodEstimationEngine : attach')
    ...
    def resume_training(engine, resume_epoch):
        print('## MaximumLikelihoodEstimationEngine : resume_training')
        engine.state.iteration = (resume_epoch - 1) * len(engine.state.dataloader)
        engine.state.epoch = (resume_epoch - 1)

    @staticmethod
    def check_best(engine):
        print('## MaximumLikelihoodEstimationEngine : check_best')
    ...
    def save_model(engine, train_engine, config, src_vocab, tgt_vocab):
        print('## MaximumLikelihoodEstimationEngine : save_model')
...
class SingleTrainer():
    ...
    def train(
        ...
        print('## SingleTrainer : train - run')
        # Start training.
        train_engine.run(train_loader, max_epochs=n_epochs)

        print('## SingleTrainer : train end')
        return model