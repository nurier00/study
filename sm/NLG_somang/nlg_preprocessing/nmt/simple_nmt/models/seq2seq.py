...
class Attention(nn.Module):
    ...
    def forward(self, h_src, h_t_tgt, mask=None):
        ...
        # torch.bmm() => [Batch, n, m] x [Batch, m, p] = [Batch, n, p]
        weight = torch.bmm(query, h_src.transpose(1, 2))
        # (bs, 1, hs) x (bs hs, length) = (bs, 1, length)
        ...
        # (bs, 1, length) x (bs, length, hs) = (bs, 1, hs)
...
class Encoder(nn.Module):
    ...
    def forward(self, emb):
        ...
        # print(f"emb : {emb}")
        # print(f"emb[0] shape : {emb[0].shape}")     # torch.Size([128, 40, 512]) => [bs, length, word_vec_size]
        # print(f"emb[1] shape : {emb[1].shape}")     # torch.Size([128]) => [bs]

        if isinstance(emb, tuple):
            x, lengths = emb
            # print(f"x shape : {x.shape}")   # torch.Size([128, 40, 512]) => [bs, length, word_vec_size]
            # print(f"lengths shape : {lengths.shape}")   # torch.Size([128]) => [bs]
            ...
            '''
            PackedSequence(
            data=tensor([[ 1.8994, -0.2072, -1.4258,  ..., -0.0495, -0.2972, -1.2039],
                [-0.1572, -1.2008,  0.4672,  ..., -0.0878,  1.1059, -0.2519],
                [-0.8268, -0.0901,  0.7046,  ...,  2.1512, -1.0516, -0.5170],
                ...,
                [-0.2717,  0.2872, -0.0370,  ...,  1.3883,  1.0116,  0.7818],
                [-0.2717,  0.2872, -0.0370,  ...,  1.3883,  1.0116,  0.7818],
                [-0.2717,  0.2872, -0.0370,  ...,  1.3883,  1.0116,  0.7818]], grad_fn=<PackPaddedSequenceBackward>),
            batch_sizes=tensor([128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128,
                128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128,
                128, 128, 128, 128, 128, 128, 128, 128, 108,  71,  47,  22]),
            sorted_indices=None,
            unsorted_indices=None)
            '''
            # print(f"packed x length : {len(x)}")            # packed x length : 4
            # print(f"packed x [0] shape : {x[0].shape}")     # torch.Size([4856, 512]) => [data, word_vec_size]
            # print(f"packed x [1] shape : {x[1].shape}")     # torch.Size([40]) => length

            # pack() => pack_padded_sequence : <pad> 토큰을 제외하고 계산하여 더 빠른 연산 처리 가능
            # x => 토큰 제거 후 합병된 데이터 / lengths.tolist() => 각 타임스텝마다 배치를 몇개 넣었는지 길이를 기록한 list
        ...
        # print(f"y[0] shape : {y[0].shape}")     # torch.Size([4856, 768]) => [data, hs]
        # print(f"y[1] shape : {y[1].shape}")     # torch.Size([40]) => length
        # print(f"y : {y}")                     # ([data,hs], [length])
        # print(f"h[0] shape : {h[0].shape}")     # torch.Size([8, 128, 384]) => [n_layers * 2, bs, hs / 2]
        # print(f"h[1] shape : {h[1].shape}")     # torch.Size([8, 128, 384]) => [n_layers * 2, bs, hs / 2]
        # print(f"h : {h}")

        if isinstance(emb, tuple):
            # unpack() => pad_packed_sequence : <pad> 토큰 제거 후 병합한 데이터를 원래 모양으로 복원
            y, _ = unpack(y, batch_first=True)
            # print(f"unpacked y : {y}")
            # print(f"unpacked y shape : {y.shape}")  # torch.Size([128, 40, 768]) => [bs, length, hs]
...
class Decoder(nn.Module):
    ...
    def forward(self, emb_t, h_t_1_tilde, h_t_1):
        ...
        # print(f"dec x shape : {x.shape}")   # torch.Size([128, 1, 1280]) => [bs, 1, dec_input_size]

        # Unlike encoder, decoder must take an input for sequentially.
        y, h = self.rnn(x, h_t_1)

        # print(f"dec y shape : {y.shape}")        # torch.Size([1, 768]) => [1, hs]
        # print(f"dec h[0] shape : {h[0].shape}")  # torch.Size([4, 128, 768]) => [n_layers, bs, hs]
        # print(f"dec h[1] shape : {h[1].shape}")  # torch.Size([4, 128, 768]) => [n_layers, bs, hs]
...
class Seq2Seq(nn.Module):
    ...
    def __init__(
        ...
        '''
        (emb_src): Embedding(8099, 512) => src.vocab_length, word_vec_size
        (emb_dec): Embedding(6249, 512) => tgt.vocab_length, word_vec_size
        '''
        ...
        '''
        (generator): Generator(
            (output): Linear(in_features=768, out_features=6249, bias=True)
    ...
    def fast_merge_encoder_hiddens(self, encoder_hiddens):
        ...
        # print(f"h_0_tgt shape : {h_0_tgt.shape}")   # torch.Size([8, 128, 384]) => [n_layers * 2, bs, hs /2]
        # print(f"c_0_tgt shape : {c_0_tgt.shape}")   # torch.Size([8, 128, 384]) => [n_layers * 2, bs, hs /2]

        batch_size = h_0_tgt.size(1)                # 128 => bs

        # contiguous() : transpose() 는 메모리를 따로 할당하지 않는 Tensor 객체 연산이므로,
        # 기존의 h_0_tgt Tensor 객체에 넣을때 사이즈가 맞지 않는 문제 발생
        # 따라서 contiguous() 를 이용하여 주소값을 바꿔주어 모양이 바뀌어도 기존 Tensor 객체에 넣을 수 있도록 해줌
        h_0_tgt = h_0_tgt.transpose(0, 1).contiguous().view(batch_size,
                                                            -1,
                                                            self.hidden_size
                                                            ).transpose(0, 1).contiguous()

        # h_0_tgt = h_0_tgt.transpose(0, 1).contiguous() => torch.Size([128, 8, 384])
        # .view(batch_size, -1, self.hidden_size) => torch.Size([128, 4, 768])
        # .transpose(0, 1).contiguous() => torch.Size([4, 128, 768]) => [n_layers, bs, hs]
    ...
    def forward(self, src, tgt):
        ...
        # print(f"src[0] shape : {src[0].shape}")     # torch.Size([128, 40]) => [bs, length]
        # print(f"src[1] shape : {src[1].shape}")     # torch.Size([128])     => [bs]

        # print(f"tgt shape : {tgt.shape}")           # torch.Size([128, 52]) => [bs, length]
        ...
        # h_0_tgt => tuple ( torch.Size([4, 128, 768]), torch.Size([4, 128, 768]) )
        emb_tgt = self.emb_dec(tgt)
        # emb_tgt shape => torch.Size([128, 57, 512])
        ...
        for t in range(tgt.size(1)):    # tgt.size(1) => length
            ...
            # emb_tgt[:, t, :] shape = torch.Size([128, 512])
            # emb_tgt[:, t, :] unsqueeze(1) shape = torch.Size([128, 1, 512])
            ...
            # print(f" torch.cat([decoder_output, context_vector], dim=-1) shape : {torch.cat([decoder_output, context_vector], dim=-1).shape}")
            # torch.Size([128, 1, 1536]) => (bs, 1, hs * 2)
            # print(f" self.concat() shape : {self.concat(torch.cat([decoder_output, context_vector], dim=-1)).shape}")
            # torch.Size([128, 1, 768]) => (bs, 1, hs)
            # print(f" h_t_tilde shape : {self.tanh(self.concat(torch.cat([decoder_output, context_vector], dim=-1))).shape}")
            # torch.Size([128, 1, 768]) => (bs, 1, hs)
            # |h_t_tilde| = (batch_size, 1, hidden_size)

            h_tilde += [h_t_tilde]
            # print(f"h_tilde shape : {h_tilde.shape}")
            # h_tilde => [tensor(), tensor(), ... ] => [(bs,1,hs), (bs,1,hs), ... ]
    ...
    def search(self, src, is_greedy=True, max_length=255):
        if isinstance(src, tuple):
            ...
            mask = None
        batch_size = x.size(0)
        # print(f"Sequence to Sequence search x shape : {x.shape}")    # torch.Size([3, 51]) => [bs, length]

        # Same procedure as teacher forcing.
        emb_src = self.emb_src(x)
        # print(f"emb_src : {emb_src.shape}")         # torch.Size([3, 51, 512]) => [bs, length, word_vec_size]

        h_src, h_0_tgt = self.encoder((emb_src, x_length))
        # print(f"h_src : {h_src.shape}, h_0_tgt[0] : {h_0_tgt[0].shape}, h_0_tgt[1] : {h_0_tgt[1].shape}")
        # h_src : torch.Size([3, 51, 768]), h_0_tgt[0] : torch.Size([8, 3, 384]), h_0_tgt[1] : torch.Size([8, 3, 384])
        # => [bs, length, hs]                  => [n_layers * 2, bs, hs / 2]            => [n_layers * 2, bs, hs / 2]

        decoder_hidden = self.fast_merge_encoder_hiddens(h_0_tgt)
        # print(f"decoder_hidden[0] shape : {decoder_hidden[0].shape}")   # torch.Size([4, 3, 768]) => [n_layers, bs, hs]
        # print(f"decoder_hidden[1] shape : {decoder_hidden[1].shape}")   # torch.Size([4, 3, 768]) => [n_layers, bs, hs]

        # Fill a vector, which has 'batch_size' dimension, with BOS value.
        y = x.new(batch_size, 1).zero_() + data_loader.BOS
        # print(f"y shape : {y.shape}")   # torch.Size([3, 1]) => [bs, 1]
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            if is_greedy:
                ...
                # print(f"y_hat.shape : {y_hat.shape}")               # torch.Size([3, 1, 5749]) => [bs, 1, output_size]
                # print(f"y_hat.exp().view(batch_size, -1) : "
                #       f"{y_hat.exp().view(batch_size, -1).shape}")  # torch.Size([3, 5749]) => [bs, output_size]
                # print(f"torch.multinomial(y_hat.exp().view(batch_size, -1), 1) : "  # torch.Size([3, 1]) => [bs, 1]
                #       f"{torch.multinomial(y_hat.exp().view(batch_size, -1), 1).shape}")

            # Put PAD if the sample is done.
            y = y.masked_fill_(~is_decoding, data_loader.PAD)
            # print(f"y shape : {y.shape}")                               # torch.Size([3,1]) => [bs, 1]

            # print(f"torch.ne(y, data_loader.EOS) :"
            #       f"{torch.ne(y, data_loader.EOS).shape}")              # torch.Size([3, 1]) => [bs, 1]
            # print(f"is_decoding * torch.ne(y, data_loader.EOS) :"
            #       f"{is_decoding * torch.ne(y, data_loader.EOS)}")      # tensor([[True], [True], [False]]) => [bs, 1]
