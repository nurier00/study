...
class MaximumLikelihoodEstimationEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        # AMP (Automatic Mixed Precision : 처리속도를 높이기 위한 FP16연산과 정확도 유지를 위한 FP32연산을 섞어 학습)
        with autocast(not engine.config.off_autocast):
            ...
            # y_hat : torch.Size([128, 57, 6249]) => [bs, length, output_size]
            # mini-batch tgt length 는 58 이었는데 <EOS> 토큰을 제거해 length 가 57 이 되었음
