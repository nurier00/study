...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        for t in range(tgt.size(1)):
            ...
            # test word print
            test_out_dec += [decoder_output]
            test_context_vector += [context_vector]

            h_tilde += [h_t_tilde]


        h_tilde1 = torch.cat(h_tilde, dim=1)
        # |h_tilde| = (batch_size, length, hidden_size)

        y_hat = self.generator(h_tilde1)
        # |y_hat| = (batch_size, length, output_size)

        return y_hat, h_src, test_out_dec, test_context_vector, h_tilde
