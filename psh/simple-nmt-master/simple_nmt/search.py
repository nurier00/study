...
class SingleBeamSearchBoard():
    ...
    def __init__(
        ...
        print("self.word_indice >> ", self.word_indice)
        print("self.word_indice[-1].unsqueeze(-1) >> ", self.word_indice[-1].unsqueeze(-1).shape)
        # Beam index for selected word index, at each time-step.
        self.beam_indice = [torch.LongTensor(beam_size).zero_().to(self.device) - 1]    # 이전 beam 이 어디서 왔나
        print("self.beam_indice >> ", self.beam_indice)
        # Cumulative log-probability for each beam.
        self.cumulative_probs = [torch.FloatTensor([.0] + [-float('inf')] * (beam_size - 1)).to(self.device)]
        print("self.cumulative_probs >>", self.cumulative_probs)
