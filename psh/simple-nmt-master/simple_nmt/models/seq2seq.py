...
class Attention(nn.Module):
    ...
    def forward(self, h_src, h_t_tgt, mask=None):
        # |h_src| = (batch_size, length, hidden_size)   인코더에서 나온 벡터값
        # |h_t_tgt| = (batch_size, 1, hidden_size)      디코더에서 나온 백터값
...
class Encoder(nn.Module):
    ...
    def __init__(self, word_vec_size, hidden_size, n_layers=4, dropout_p=.2):
        ...
        self.rnn = nn.LSTM(
            word_vec_size,  # rnn 입력 크기
            int(hidden_size / 2),  # rnn 출력 크기
            num_layers=n_layers,
            dropout=dropout_p,
            bidirectional=True,
            batch_first=True,
    ...
    def forward(self, emb):
        ...
        if isinstance(emb, tuple):
            x, lengths = emb
            # pad가 포함된 x와 각 문장별 길이를 가지고 있는 lengths를 통해 packking 진행
            # pad는 학습하지 않기 위해 실제 데이터 있는 부분만 학습
...
class Decoder(nn.Module):
    ...
    def forward(self, emb_t, h_t_1_tilde, h_t_1):
        # |emb_t| = (batch_size, 1, word_vec_size)  디코더의 y입력
        # |h_t_1_tilde| = (batch_size, 1, hidden_size)  디코더의 rnn 결과 입력
        # |h_t_1[0]| = (n_layers, batch_size, hidden_size)  전 레이어의 h_state 입력
        batch_size = emb_t.size(0)
        hidden_size = h_t_1[0].size(-1)

        if h_t_1_tilde is None:
            # If this is the first time-step,
            # emb_t 와 cat 하기 위해 같은 size 로 생성
...
class Seq2Seq(nn.Module):
    ...
    # 디코더에서 어텐션사용 할 때 뒤에 pad들을 무한대 세팅하기 위한 패딩처리
    def generate_mask(self, x, length):
        mask = []

        max_length = max(length)    # 배치사이즈 내 각 문장들의 길이 리스트
        for l in length:
            if max_length - l > 0:
    ...
    def search(self, src, is_greedy=True, max_length=255):
        ...
        is_decoding = x.new_ones(batch_size, 1).bool()  # 1로 새 tensor 만들고 true 로 변경
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            # is_decoding == false 면 PAD 추가
            y = y.masked_fill_(~is_decoding, data_loader.PAD)
            # Update is_decoding if there is EOS token.
            # torch.ne > tensor의 값 비교해서 다르면 true
            # is_decoding 은 true로 초기화 되있음, 한 문장이 끝나서 EOS를 만나면 is_decoding은 false로 된다
    ...
    # @profile
    # translate.py 에서 config.beam_size > 1 경우 실행
    def batch_beam_search(
            self,
            src,                # 단어사전에 의해 indexing 된 배치사이즈 만큼의 문장들, 내림차순정렬, <pad> 추가상태
            beam_size=5,
            max_length=255,
            n_best=1,
            length_penalty=.2
        ...
        h_0_tgt = self.fast_merge_encoder_hiddens(h_0_tgt)
        # |h_0_tgt| = (n_layers, batch_size, hidden_size)
        # 여기 까진 일반 search 와 같음

        # initialize 'SingleBeamSearchBoard' as many as batch_size
        # 병렬로 진행하기 위해 배치사이즈 만큼 SingleBeamSearchBoard 생성
        boards = [SingleBeamSearchBoard(
            h_src.device,
            {
                'hidden_state': {
                    'init_status': h_0_tgt[0][:, i, :].unsqueeze(1),    # i로 slice 하면 3차원에서 2차원이 되므로 unsqueeze 진행
                    'batch_dim_index': 1,
        ...
        # beam_search 진행이 다 끝났는지 확인, 다 끝나면 1
        ...
        while sum(is_done) < batch_size and length <= max_length:
            ...
            # 한 문장씩 반복, board 하나에 한 문장
            for i, board in enumerate(boards):
                # Batchify if the inference for the sample is still not finished.
                # 번역이 아직 끝나지 않았다면,
                # 즉 EOS 가 아니라면 이전 time 까지 구한 hat값에 현재 time에 구할 hat이 들어갈 tensor size 할당
                if board.is_done() == 0:
                    y_hat_i, prev_status = board.get_batch()
                    # |y_hat_i| = (beam_size, 1)  <BOS> 부터 시작하는 현재 time_stamp 의 y_hat 이 beam 개수만큼

                    hidden_i = prev_status['hidden_state']
                    cell_i = prev_status['cell_state']
                    h_t_tilde_i = prev_status['h_t_1_tilde']

                    # 이전 time 까지 추론한 y_hat 값들
                    fab_input += [y_hat_i]
                    fab_hidden += [hidden_i]
                    fab_cell += [cell_i]

                    # 현재 time 에 추론할 hat 값의 tensor 크기 설정
                    # 첫번째 차원(batch_size)  beam_size 배 만큼 커지게
            ...
            # batch_size 기준으로 cat 진행
            # cat 하면 지정한 차원 size 가 커진다
            fab_input = torch.cat(fab_input, dim=0)
            fab_hidden = torch.cat(fab_hidden, dim=1)
            fab_cell = torch.cat(fab_cell, dim=1)

            # stack 하면 차원이 확장 된다 default = 0차원
            # fab_h_src list의 길이만큼 dim=0 차원이 확장
            # fab_h_src len >>  6
            # before stack fab_h_src shape >>  torch.Size([51, 768])
            # after stack fab_h_src shape >>  torch.Size([6, 51, 768])
            ...
            cnt = 0
            # 인코딩단계에서 batch * beam 작업 한 것을 다시 beam size 단위로 쪼개서 best 구한다
            for board in boards:
                if board.is_done() == 0:
        ...
        print("batch_sentences >> ", batch_sentences[0])
        print("batch_sentences >> ", batch_sentences[1])
        print("batch_probs >> ", batch_probs[0])
        print("batch_probs >> ", batch_probs[1])
