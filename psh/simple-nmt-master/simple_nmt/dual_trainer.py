...
class DualSupervisedTrainingEngine(Engine):
    ...
    def _get_loss(x, y, x_hat, y_hat, crits, x_lm=None, y_lm=None, lagrange=1e-3):
        ...
        if x_lm is not None and y_lm is not None:
            ...
            # x -> y 에서는 p(x|y) 는 grad 계산 제외 하기 위해 detach 사용
    ...
    def train(engine, mini_batch):
        ...
        with autocast(not engine.config.off_autocast):
            ...
            # |x| = (batch_size, n)
            # |y| = (batch_size, m)
            y_hat = engine.models[X2Y](x, y)    # seq2seq forward result
            # |y_hat| = (batch_size, m, y_vocab_size)

            # dsl_n_warmup_epochs 이후에만 타는데 뭐지?
            ...
            x, y, restore_indice = DualSupervisedTrainingEngine._reorder(
                mini_batch.src[0][:, :-1],    # <BOS>포함 <EOS>미포함
                mini_batch.tgt[0][:, 1:-1],   # <BOS>, <EOS> 미포함
                mini_batch.tgt[1] - 2,        # <BOS>,<EOS> 제외 한 문장 길이
            )
            # y 기준 문장길이로 내림차순 한 batch
            # |x| = (batch_size, n)
            # |y| = (batch_size, m)

            # x_hat 을 문장길이 내림차순 정렬하기 위해 _restore_order 사용
